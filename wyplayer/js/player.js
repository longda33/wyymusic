...
if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) $("#wenkmPlayer").hide();
    ...
    apiurl = "https://www.xiaozouwl.club/wyplayer/";
    fileurl = apiurl + "wy_list.php?mode=list&parm=" + user;
    ...
    var cicleTime = null;
    $cover.html('<img src="https://q.qlogo.cn/headimg_dl?bs=qq&dst_uin=824986221&src_uin=tangrui.wang&fid=blog&spec=100">');
    $songName.html('<a style="color:#f00">初始化失败</a>');
    $songFrom.html('<a style="color:#f00">加载失败</a>');
    $songFrom1.html('<a style="color:#f00">TangRui</a>');
    ...
    var wenkmMedia = {
        ...
        getInfos: function(a) {
            ...
            audio.src = apiurl + "wy_song.php?mode=net_mp3&parm=" + NeiCeList[albumId].song_list[songId].song_id;
            $songFrom5.show();
            ...
            lrcurl = apiurl + "wy_lrc.php?mode=net_lrc&parm=" + id;
            ...
            var imgscr = NeiCeList[albumId].song_list[songId].pic;
            if (protocolStr == "http:") {
                c.src = imgscr;
            } else if (protocolStr == "https:") {
                c.src = imgscr.replace("http://", "//");
            ...
            c.onload = function() {
                ...
                100);
                $.ajax({
                    url: apiurl + "wy_color.php?mode=image_info&parm=" + base64_encode(c.src),
                    type: "GET",
                    dataType: "script",
                    success: function() {
                        $(".myhk_pjax_loading_frame,.myhk_pjax_loading").hide();
            ...
            c.error = function() {
                ...
                800);
                c.src = "https://q.qlogo.cn/headimg_dl?bs=qq&dst_uin=840160384&src_uin=tangrui.wang&fid=blog&spec=100";
                setTimeout(function() {
    ...
    wenkmPlayer.newplayList = {
        creat: {
            album: function() {
                ...
                for (var b = 0; b < a; b++) c += '<li><i class="fa fa-angle-right"></i><span class="index">' + (b + 1) + '</span><span class="artist"></span>' + NeiCeList[b].song_album + "</li>";
    ...
    wenkmLrc = {
        load: function() {
            ...
            300) : $(".switch-ksclrc").show();
            $.ajax({
                url: apiurl + "wy_lrc.php?mode=net_lrc&parm=" + id,
                type: "GET",
                dataType: "script",
                success: function() {
...
function LimitStr(a, c, b) {
    ...
    e = a.length,
    f = 0,
    g = 0; f < 2 * c && g < e; g++) f += 128 < a.charCodeAt(g) ? 2 : 1,
    d += a.charAt(g);
    g < e && (d += b);
    return d
}
...
function playercolor(imginfo) {
    ...
    // $('#wenkmLrc ul li').css({
    // 	color: "rgba(" + imginfo[0].img_color + ",.8)"
    // });
}
...
function get_list(a, c) {
    $("#wenkmLrc").show();
    $(".music_loading_frame,.music_loading").show();
    $.ajax({
        url: apiurl + "wy_list.php?mode=list&parm=" + a + "",
        type: "GET",
        dataType: "script",
        success: function() {
            setTimeout(function() {
            ...
            500);
            setTimeout(function() {
                $player.addClass("showAlbumList")
            },
            1E3);
            wenkmPlayer.playList.creat.album();
            $(".switch-default").show();
            c && wenkmMedia.getInfos(c - 1)
        },
        error: function(a, c, f) {
            wenkmTips.show("\u6b4c\u66f2\u5217\u8868\u83b7\u53d6\u5931\u8d25!");
            $(".music_loading_frame,.music_loading").hide()
        }
    })
}
